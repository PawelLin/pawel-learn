    ...
    <section class="timi">
        <section class="contain" :style="{ transform: `translate3d(${translateX}%, 0, 0)` }">
            <div v-for="({ year, months, skinsLength, herosLength }, index1) in list" :key="year">
                ...
                <div v-for="({ skins, title, heros }, index2) in months" :key="`${year}${index2}`" v-show="showYears.includes(year)">
                    <div class="skin">
...
export default defineComponent({
    setup() {
        ...
        datas.forEach(list => {
            list.forEach((item, index) => {
                if (item.date) {
                    if (!index) {
                        if (!heros[item.date]) {
                            heros[item.date] = { alt: [], src: [] }
                        }
                        heros[item.date].alt.push(`${item.name} [ ${item.date} ]`)
                        heros[item.date].src.push(item.icon)
                    } else {
                        if (!skins[item.date]) {
                            skins[item.date] = { alt: [], src: [] }
                        }
                        skins[item.date].alt.push(`${item.name} [ ${item.date} ]`)
                        skins[item.date].src.push(item.icon)
                    }
                } else {
        ...
        list[0].months = list[0].months.filter(item => item.skins.length || item.heros.length)
